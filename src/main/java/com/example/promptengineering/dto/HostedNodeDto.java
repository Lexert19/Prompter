...
public class HostedNodeDto {
    ...
    public static HostedNodeDto fromEntity(HostedNode node) {
        return HostedNodeDto.builder().id(node.getId()).nodeName(node.getNodeName())
                .modelName(node.getModelName()).modelFamily(node.getModelFamily())
                .authToken(node.getAuthToken()).status(node.getStatus())
                .lastHeartbeat(node.getLastHeartbeat())
                .clientVersion(node.getClientVersion()).publicIp(node.getPublicIp())
                .pointsPer1kInput(node.getPointsPer1kInput())
                .pointsPer1kOutput(node.getPointsPer1kOutput())
                .totalRequests(node.getTotalRequests()).totalTokens(node.getTotalTokens())
                .allowPublicUse(node.isAllowPublicUse()).build();
    }
}
