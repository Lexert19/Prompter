...
public class PointsColumnsMigration implements CommandLineRunner {
    ...
    public PointsColumnsMigration(MigrationFlagRepository migrationFlagRepository, JdbcTemplate jdbcTemplate) {
    ...
    public void run(String... args) {
        ...
        if (!columnExists("app_user", "points")) {
            jdbcTemplate.execute("ALTER TABLE app_user ADD COLUMN points DOUBLE PRECISION");
        ...
        if (!columnExists("model", "points_per_input")) {
            jdbcTemplate.execute("ALTER TABLE model ADD COLUMN points_per_input DOUBLE PRECISION");
            jdbcTemplate.update("UPDATE model SET points_per_input = 0 WHERE points_per_input IS NULL");
            jdbcTemplate.execute("ALTER TABLE model ALTER COLUMN points_per_input SET NOT NULL");
        } else {
            jdbcTemplate.update("UPDATE model SET points_per_input = 0 WHERE points_per_input IS NULL");
        }

        if (!columnExists("model", "points_per_output")) {
            jdbcTemplate.execute("ALTER TABLE model ADD COLUMN points_per_output DOUBLE PRECISION");
            jdbcTemplate.update("UPDATE model SET points_per_output = 0 WHERE points_per_output IS NULL");
            jdbcTemplate.execute("ALTER TABLE model ALTER COLUMN points_per_output SET NOT NULL");
        } else {
            jdbcTemplate.update("UPDATE model SET points_per_output = 0 WHERE points_per_output IS NULL");
        }
